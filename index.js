...
const urlImages = "https://api.thecatapi.com/v1/favourites";
 ...
 */
breedSelect.addEventListener('change', axiosHandlerBreedSelect);
async function handleBreedSelect() {
  ...
  breedData.forEach(obj => {
    const ele = Carousel.createCarouselItem(obj.url, obj.breeds[0].name, obj.id)
    infoDump.innerHTML = obj.breeds[0].description;
...
async function axiosHandlerBreedSelect() {
  ...
  breedsData.forEach(item => {
    ...
    infoDump.innerHTML = item.breeds[0].description;
 ...
 axios.get(urlImages).then((res) => {
  ...
  res.data.forEach((item) => {
    if (item.image_id === imgId) {
      deleted = true;
      axios.delete(`${urlImages}/${item.id}`);
    } 
    // else {
    //   axios.post(urlImages, {image_id: imgId})
    // }
  });
  if (!deleted) {
    axios.post(urlImages, {image_id: imgId, sub_id: 'abe'}).then((res) => console.log(res.data)
    );
  }
  
 });
  
  // axios.post(urlImages, {"image_id": imgId}).then(res => console.log(res.data)
